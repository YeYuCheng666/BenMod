    ...
    public void update() {
        if (this.target != null && this.target.hasPower("BenMod:SinPower")) {
            if (AbstractDungeon.player.hasRelic("BenMod:DemonSlayer")){
                this.addToTop(new ApplyPowerAction(this.target, this.source, new SinPower(this.target, this.target.getPower("BenMod:SinPower").amount * 2 + 1), this.target.getPower("BenMod:SinPower").amount * 2 + 1));
            }else{
